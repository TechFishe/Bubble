    ...
    <div class="flex h-fullscreen">
        ...
        <main v-if="currentFriend.full_name != ''" class="2xl:w-5/6 w-9/12 flex flex-col pb-2">
            ...
            <div class="mr-8 h-8 flex">
                <input type="text" v-model="msg" placeholder="Enter message" class="w-full bg-shark-900 text-lg rounded-l-sm pl-2 h-8 placeholder:italic outline-0">
                <button @keypress.enter="sendChat" @click="sendChat" :disabled="msg === ''" class="bg-shark-900 rounded-r-sm pr-2 text-snow/45 enabled:hover:text-aero-200 transition-colors duration-200 ease-out">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">

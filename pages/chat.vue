        ...
        <main v-if="currentFriend.full_name != '' || currentGroup.group_name != ''" class="2xl:w-5/6 w-9/12 flex flex-col pb-2">
            ...
            <ul id="chatbox" class="ml-2 pb-2 overflow-y-scroll max-h-chatView flex w-screen flex-col-reverse">
                <li v-if="currentFriend.full_name != ''" v-for="chat in chats" class="flex space-x-2 max-w-[61%] group">
                    ...
                    <button v-show="chat.sent_by === customUser.user_id" @click="deleteChat(chat)" class="hover:text-red-400 hidden group-hover:flex transition-colors duration-200 ease-in">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">

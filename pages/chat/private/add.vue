...
<template>
  <main class="flex w-3/4 items-center justify-center">
    <div class="flex w-5/6 flex-col items-center space-y-2 rounded-lg bg-shark-950 px-2 py-2">
      <h1 class="border-b-[3px] border-b-snow/15 bg-gradient-to-r from-snow to-aero-100 to-65% bg-clip-text px-4 font-mono text-7xl font-black uppercase text-transparent">Add Friend</h1>
      ...
      <ul class="grid w-full grid-cols-4 grid-rows-5">
        <li v-for="user in list" class="flex items-center border-r border-r-snow/45 px-1 py-0.5 last:border-r-transparent">
          ...
          <button @click="addFriend(user)" class="transition-colors duration-200 ease-in hover:text-aero-200">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">

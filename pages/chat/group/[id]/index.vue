...
<script setup lang="ts">
  ...
  let groupChatChannel: RealtimeChannel;
  ...
  onMounted(() => {
    ...
    function getChannels() {
      ...
      let groupMsgFilter = `sent_to=eq.${route.params.id}`;
...
<template>
  <div class="flex h-fullscreen">
    <Sidebar />
    <main class="flex w-3/4 flex-col pb-2">
      <section class="ml-1 flex h-[5.5rem] w-screen items-center">
        <img :src="group.pfp" alt="Friend pfp" width="48px" height="48px" />
        <h1 class="mx-1 h-fit w-fit bg-gradient-to-r from-snow to-aero-100 to-65% bg-clip-text font-mono text-6xl font-semibold leading-snug text-transparent">{{ group.group_name }}</h1>
        <div class="flex items-center space-x-3">
          <NuxtLink :to="`/chat/group/${$route.params.id}/add`" class="transition-all duration-200 ease-out hover:text-aero-100 hover:drop-shadow-navBtn">
      ...
      <ul id="noScrollbar" class="ml-2 flex max-h-chatView w-screen flex-col-reverse overflow-y-scroll pb-2">
        <li v-for="chat in chats" class="group flex w-fit space-x-2 rounded-md px-2 py-px transition-all duration-100 ease-out hover:cursor-default hover:bg-shark-900">
          ...
          <p v-else>
            <span class="text-sky-300">{{ checkId(chat) }}</span
            >:
          </p>
